	...
	FString& OutError)
{
	...
	for (int32 RuleIndex = 0; RuleIndex < Rules.Num(); RuleIndex++)
	{
		...
		for (UAnimStateTransitionNode* Transition : AllTransitions)
		{
			...
			int32 PosX = UnrealCodexConstants::AnimDiagram::ConditionNodeStartX;
			int32 PosY = UnrealCodexConstants::AnimDiagram::ConditionNodeStartY;
				...
				if (CondType.Equals(TEXT("TimeRemaining"), ESearchCase::IgnoreCase))
				{
					...
					else
					{
						...
						UEdGraphNode* CompNode = AddConditionNode(
							AnimBP, StateMachineName, FromStateName, ToStateName,
							Comparison, nullptr, FVector2D(PosX + UnrealCodexConstants::AnimDiagram::ConditionNodeSpacing, PosY),
	...
	const TArray<FDiagramTransition>& Transitions)
{
	...
	const int32 CellWidth = UnrealCodexConstants::AnimDiagram::DiagramCellWidth;
	const int32 CellHeight = UnrealCodexConstants::AnimDiagram::DiagramCellHeight;
	...
	for (int32 y = 0; y <= MaxY; y++)
	{
		...
		for (int32 x = 0; x <= MaxX; x++)
		{
			if (const FDiagramState* const* StatePtr = StateGrid.Find(TPair<int32, int32>(x, y)))
			{
				...
				if (StateName.Len() > UnrealCodexConstants::AnimDiagram::MaxStateNameDisplayLength)
				{
					StateName = StateName.Left(UnrealCodexConstants::AnimDiagram::MaxStateNameDisplayLength - 1) + TEXT(".");
				}
