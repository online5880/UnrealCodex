	...
	, ScriptCounter(0)
{
	...
	if (!IFileManager::Get().DirectoryExists(*ContentScriptDir))
	{
		...
		UE_LOG(LogUnrealCodex, Log, TEXT("Created script directory: %s"), *ContentScriptDir);
	}
}
	...
	const FString& Description)
{
	...
	UE_LOG(LogUnrealCodex, Log, TEXT("C++ script written to: %s"), *FilePath);
	...
	UE_LOG(LogUnrealCodex, Warning, TEXT("C++ compilation failed, returning error for Codex to fix"));
...
bool FScriptExecutionManager::TriggerLiveCodingCompile(FString& OutErrorLog)
{
	...
	if (OutputCapture.bHasErrors)
	{
		...
		UE_LOG(LogUnrealCodex, Error, TEXT("Live Coding compilation failed:\n%s"), *OutErrorLog);
		return false;
	}

	UE_LOG(LogUnrealCodex, Log, TEXT("Live Coding compilation completed successfully"));
	return true;
#else
	...
	const FString& Description)
{
	...
	FUnrealCodexOutputDevice ExecOutput;
	FUnrealCodexOutputDevice LogOutput;
	...
	UE_LOG(LogUnrealCodex, Log, TEXT("Python script output (%d chars): %s"),
		Output.Len(), Output.Len() > 500 ? *(Output.Left(500) + TEXT("...")) : *Output);
	UE_LOG(LogUnrealCodex, Log, TEXT("Python script actor delta: %d before, %d after (%+d)"),
	...
	const FString& Description)
{
	...
	FUnrealCodexOutputDevice OutputDevice;
	FString AllOutput;
	...
	for (const FString& RawCommand : Commands)
	{
		...
		if (Command.Contains(TEXT("@UnrealCodex")) || Command.Contains(TEXT("@Name:")) ||
	...
	const FString& ScriptName)
{
	...
	UE_LOG(LogUnrealCodex, Error, TEXT("Failed to write script to: %s"), *FilePath);
	return FString();
}
...
bool FScriptExecutionManager::SaveHistory()
{
	...
	{
		UE_LOG(LogUnrealCodex, Log, TEXT("Script history saved: %d entries"), History.Num());
		return true;
	}

	UE_LOG(LogUnrealCodex, Error, TEXT("Failed to save script history to: %s"), *HistoryPath);
	return false;
}
...
bool FScriptExecutionManager::LoadHistory()
{
	...
	{
		UE_LOG(LogUnrealCodex, Log, TEXT("No script history file found"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to load script history from: %s"), *HistoryPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to parse script history JSON"));
		return false;
	}
	...
	UE_LOG(LogUnrealCodex, Log, TEXT("Loaded script history: %d entries"), History.Num());
	return true;
}
...
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealCodex"), TEXT("script_history.json"));
}

FString FScriptExecutionManager::GetCppScriptDirectory() const
{
	// Source/[ProjectName]/Generated/UnrealCodex/
	FString ProjectName = FApp::GetProjectName();
	return FPaths::Combine(FPaths::ProjectDir(), TEXT("Source"), ProjectName, TEXT("Generated"), TEXT("UnrealCodex"));
}

FString FScriptExecutionManager::GetContentScriptDirectory() const
{
	// Content/UnrealCodex/Scripts/
	return FPaths::Combine(FPaths::ProjectContentDir(), TEXT("UnrealCodex"), TEXT("Scripts"));
}
