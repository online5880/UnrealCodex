...
FCodexSessionManager::FCodexSessionManager()
	: MaxHistorySize(UnrealCodexConstants::Session::MaxHistorySize)
{
}

void FCodexSessionManager::AddExchange(const FString& Prompt, const FString& Response)
{
...
void FCodexSessionManager::ClearHistory()
{
	ConversationHistory.Empty();
}

FString FCodexSessionManager::GetSessionFilePath() const
{
	FString SaveDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealCodex"));
	return FPaths::Combine(SaveDir, TEXT("session.json"));
}

bool FCodexSessionManager::HasSavedSession() const
{
	return IFileManager::Get().FileExists(*GetSessionFilePath());
}

bool FCodexSessionManager::SaveSession()
{
	...
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to serialize session JSON"));
		return false;
	}

	if (!FFileHelper::SaveStringToFile(JsonString, *SessionPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to save session to: %s"), *SessionPath);
		return false;
	}

	UE_LOG(LogUnrealCodex, Log, TEXT("Session saved to: %s (%d messages)"), *SessionPath, ConversationHistory.Num());
	return true;
}

bool FCodexSessionManager::LoadSession()
{
	FString SessionPath = GetSessionFilePath();

	if (!IFileManager::Get().FileExists(*SessionPath))
	{
		UE_LOG(LogUnrealCodex, Log, TEXT("No previous session found at: %s"), *SessionPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to load session from: %s"), *SessionPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to parse session JSON"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Log, TEXT("Session loaded from: %s (last updated: %s, %d messages)"),
			*SessionPath, *LastUpdated, ConversationHistory.Num());
	}
	else
	{
		UE_LOG(LogUnrealCodex, Log, TEXT("Session loaded from: %s (%d messages)"),
