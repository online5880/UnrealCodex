...
void FProjectContextManager::RefreshContext()
{
	...
	UE_LOG(LogUnrealCodex, Log, TEXT("Refreshing project context..."));
	...
	UE_LOG(LogUnrealCodex, Log, TEXT("Project context gathered:"));
	UE_LOG(LogUnrealCodex, Log, TEXT("  - Source files: %d"), CachedContext.SourceFiles.Num());
	UE_LOG(LogUnrealCodex, Log, TEXT("  - UCLASS types: %d"), CachedContext.UClasses.Num());
	UE_LOG(LogUnrealCodex, Log, TEXT("  - Level actors: %d"), CachedContext.LevelActors.Num());
	UE_LOG(LogUnrealCodex, Log, TEXT("  - Total assets: %d"), CachedContext.AssetCount);
}
...
void FProjectContextManager::ScanSourceFiles()
{
	...
	{
		UE_LOG(LogUnrealCodex, Warning, TEXT("Source directory not found: %s"), *SourceDir);
		return;
	}
...
bool FProjectContextManager::ParseSingleUClass(const FString& FileContent, const FString& RelativePath, int32 UClassPos, int32& OutNextSearchPos)
{
	...
	if (ClassPos == INDEX_NONE || ClassPos > UClassPos + UnrealCodexConstants::Context::MaxUClassToClassKeywordDistance)
	{
	...
	if (InheritPos != INDEX_NONE && InheritPos < ClassNameEnd + UnrealCodexConstants::Context::MaxClassNameToInheritanceDistance)
	{
		int32 ParentEnd;
...
FString FProjectContextManager::FormatContextForPrompt() const
{
	...
	if (CachedContext.UClasses.Num() > 0)
	{
		...
		int32 MaxClasses = FMath::Min(CachedContext.UClasses.Num(), UnrealCodexConstants::Context::MaxClassesToFormat);
	...
	if (CachedContext.SourceFiles.Num() > 0)
	{
		...
		{
			if (DirCount++ >= UnrealCodexConstants::Context::MaxDirectoriesToShow)
			{
				Context += FString::Printf(TEXT("  ... and %d more directories\n"), FilesByDir.Num() - UnrealCodexConstants::Context::MaxDirectoriesToShow);
				break;
			}
