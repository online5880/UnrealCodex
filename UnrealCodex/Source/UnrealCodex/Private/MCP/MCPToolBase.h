...
protected:
		...
		const FVector& DefaultValue = FVector::ZeroVector) const
	{
		...
		{
			return UnrealCodexJsonUtils::ExtractVector(*VectorObj, DefaultValue);
		}
		return DefaultValue;
	}
		...
		const FRotator& DefaultValue = FRotator::ZeroRotator) const
	{
		...
		{
			return UnrealCodexJsonUtils::ExtractRotator(*RotatorObj, DefaultValue);
		}
		return DefaultValue;
	}
	...
	TSharedPtr<FJsonObject> BuildActorInfoWithTransformJson(AActor* Actor) const
	{
		...
		if (Actor)
		{
			ActorJson->SetObjectField(TEXT("location"), UnrealCodexJsonUtils::VectorToJson(Actor->GetActorLocation()));
			ActorJson->SetObjectField(TEXT("rotation"), UnrealCodexJsonUtils::RotatorToJson(Actor->GetActorRotation()));
			ActorJson->SetObjectField(TEXT("scale"), UnrealCodexJsonUtils::VectorToJson(Actor->GetActorScale3D()));
		}
		return ActorJson;
	}
