...
FMCPToolResult FMCPTool_MoveActor::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	ResultData->SetObjectField(TEXT("location"), UnrealCodexJsonUtils::VectorToJson(Actor->GetActorLocation()));
	ResultData->SetObjectField(TEXT("rotation"), UnrealCodexJsonUtils::RotatorToJson(Actor->GetActorRotation()));
	ResultData->SetObjectField(TEXT("scale"), UnrealCodexJsonUtils::VectorToJson(Actor->GetActorScale3D()));
