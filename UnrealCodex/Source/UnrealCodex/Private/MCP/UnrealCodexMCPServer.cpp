...
#include "UnrealCodexMCPServer.h"
...
FUnrealCodexMCPServer::FUnrealCodexMCPServer()
	: bIsRunning(false)
	, ServerPort(UnrealCodexConstants::MCPServer::DefaultPort)
{
...
bool FUnrealCodexMCPServer::Start(uint32 Port)
{
	if (bIsRunning)
	{
		UE_LOG(LogUnrealCodex, Warning, TEXT("MCP Server is already running on port %d"), ServerPort);
		return true;
	}
	...
	{
		UE_LOG(LogUnrealCodex, Error, TEXT("Failed to get HTTP router for port %d"), ServerPort);
		return false;
	}
	...
	UE_LOG(LogUnrealCodex, Log, TEXT("MCP Server started on http://localhost:%d"), ServerPort);
	UE_LOG(LogUnrealCodex, Log, TEXT("  GET  /mcp/tools      - List available tools"));
	UE_LOG(LogUnrealCodex, Log, TEXT("  POST /mcp/tool/{name} - Execute a tool"));
	UE_LOG(LogUnrealCodex, Log, TEXT("  GET  /mcp/status     - Server status"));

	return true;
}

void FUnrealCodexMCPServer::Stop()
{
	if (!bIsRunning)
	{
		return;
	...
	bIsRunning = false;
	UE_LOG(LogUnrealCodex, Log, TEXT("MCP Server stopped"));
}

void FUnrealCodexMCPServer::SetupRoutes()
{
	if (!HttpRouter.IsValid())
	{
		return;
	...
	ListToolsHandle = HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FUnrealCodexMCPServer::HandleListTools)
	);
	...
	ExecuteToolHandle = HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FUnrealCodexMCPServer::HandleExecuteTool)
	);
	...
	StatusHandle = HttpRouter->BindRoute(
		...
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FUnrealCodexMCPServer::HandleStatus)
	);
}

bool FUnrealCodexMCPServer::HandleListTools(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
...
bool FUnrealCodexMCPServer::HandleExecuteTool(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	if (Request.Body.Num() > UnrealCodexConstants::MCPServer::MaxRequestBodySize)
	{
		UE_LOG(LogUnrealCodex, Warning, TEXT("Request body too large: %d bytes (max %d)"), Request.Body.Num(), UnrealCodexConstants::MCPServer::MaxRequestBodySize);
	...
	if (Request.Body.Num() > 0)
	{
		...
		{
			UE_LOG(LogUnrealCodex, Warning, TEXT("Failed to parse JSON body: %s"), *BodyString);
...
bool FUnrealCodexMCPServer::HandleStatus(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	TSharedPtr<FJsonObject> ResponseJson = MakeShared<FJsonObject>();
...
TUniquePtr<FHttpServerResponse> FUnrealCodexMCPServer::CreateJsonResponse(const FString& JsonContent, EHttpServerResponseCodes Code)
{
...
TUniquePtr<FHttpServerResponse> FUnrealCodexMCPServer::CreateErrorResponse(const FString& Message, EHttpServerResponseCodes Code)
{
